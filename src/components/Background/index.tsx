...
const lerp = (percent: number, lastValue: number, nextValue: number) => {
  return lastValue + percent * (nextValue - lastValue)
}

const lerpArr = (percent: number, lastValues: number[], nextValues: number[]) =>
  lastValues.map((v, i) => lerp(percent, v, nextValues[i]))

const lerpMultiple = (percent: number, points: string | any[], fn = lerp) => {
...
const ParticleAnimation = () => {
  ...
  useEffect(() => {
        ...
        for (let z = 0; z < particlesPerAxis; z++) {
          vertices.push(
            -1200 +
              ...
              simplex.noise2D((x / particlesPerAxis) * 0.1, 1) *
                (particlesPerAxis * 10) +
              Math.random() * (80 * multiplier),
            -1000 +
              (y / particlesPerAxis) * 2000 +
              simplex.noise2D(1, (y / particlesPerAxis) * 0.1) *
                (particlesPerAxis * 10) +
              Math.random() * 70,
            simplex.noise3D(
              x / particlesPerAxis,
    ...
    const createColors = (vertices: any) => {
      const colors = []
    ...
    function createCanvasMaterial(color: string, size = 256) {
